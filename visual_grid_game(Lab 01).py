...
class VisualGridHuntGame:
    ...
    def __init__(
        ...
        # Declare & populate toxic traps avoiding (0, 0), walls, and food
    ...
    def get_percept(self) -> dict:
        return {
            ...
            'on_trap': tuple(self.agent_pos) in self.toxic_traps,
    ...
    def execute_action(self, action: str):
        ...
        # Check interaction with food
        if tuple_pos in self.food_positions:
            self.food_positions.remove(tuple_pos)
            self.score += 20

        # Check interaction with toxic traps (Decrement score by 15 points)
    ...
    def is_done(self) -> bool:
        return (
            len(self.food_positions) == 0 or self.steps >= 60 or self.collision
        )
...
class GridGameGUI:
    ...
    def __init__(
        ...
        # Dynamically calculate cell size so the total canvas fits within a 600x600 ceiling
        max_canvas_dim = 600
        self.cell_size = max(
            20,
            min(
    ...
    def draw_grid(self):
        ...
        for x in range(self.env.width):
            for y in range(self.env.height):
                ...
                color = (
                    "#f1f5f9" if (x, y) not in self.env.walls else "#64748b"
                )
        ...
        # Render custom purple shapes for toxic traps on the canvas
        for tx, ty in self.env.toxic_traps:
            offset = self.cell_size * 0.2
            x1 = tx * self.cell_size + offset
            y1 = (self.env.height - 1 - ty) * self.cell_size + offset
            x2 = (tx + 1) * self.cell_size - offset
            y2 = (self.env.height - ty) * self.cell_size - offset
            # Custom purple rectangle with dark border
            self.canvas.create_rectangle(
                x1, y1, x2, y2, fill="#7e22ce", outline="#581c87"
            )

        # Draw food (Yellow/Orange ovals)
